...
GameWidget::GameWidget(QWidget *parent)
  ...
  layout->addWidget(new QLabel("Число компьютера:" , this), 0, 2);
  layout->addWidget(&labelCompNumber, 1, 2);

  layout->addWidget(new QLabel("Введите число", this), 2, 0);
  layout->addWidget(&numberInput, 2, 1);
  layout->addWidget(&checkButton, 2, 2);
  layout->addWidget(&readNumButton, 2, 2);

  layout->addWidget(&table, 3, 0, 1, 3);
  layout->addWidget(new QLabel("Ходы компьютера", this), 5, 1);
  layout->addWidget(&ai_table, 6, 0, 1, 3);
...
void GameWidget::makeMove() {
    ...
    if(sValue != computerNumber){
        ...
        personWon = true;
        labelCompNumber.setText(computerNumber);
        gameStop();
    }
}
...
void GameWidget::computerMove(){
    ...
    int nBulls = 0, nCows = 0, nBullsCorrectValue = 0, nCowsCorrectValue = 0;
    calcBullsAndCows(sValue, personNumber, nCowsCorrectValue, nBullsCorrectValue);

    bool pressedOk;
    nBulls = QInputDialog::getText(this, tr("Число быков"), "Комьютер ввел число " + sValue + ". Число быков:",  QLineEdit::Normal, QString(), &pressedOk).toInt();
    while(nBulls != nBullsCorrectValue || !pressedOk){
        nBulls = QInputDialog::getText(this, tr("Число быков"), "Некорректное число быков. Повторите ввод.\nКомьютер ввел число " + sValue + ". Число быков:",  QLineEdit::Normal, QString(), &pressedOk).toInt();
    }

    nCows = QInputDialog::getText(this, tr("Число коров"), "Комьютер ввел число " + sValue + ". Число коров:",  QLineEdit::Normal, QString(), &pressedOk).toInt();
    while(nCows != nCowsCorrectValue || !pressedOk){
        nCows = QInputDialog::getText(this, tr("Число коров"), "Некорректное число коров. Повторите ввод.\nКомьютер ввел число " + sValue + ". Число коров:",  QLineEdit::Normal, QString(), &pressedOk).toInt();
    }
