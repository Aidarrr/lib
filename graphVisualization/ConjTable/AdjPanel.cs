...
namespace System.Windows.Forms
{
    public class AdjPanel : Panel
    {
        ...
        public void Build(int[,] conjMatrix, List<Tuple<string, double>> coefficients)
        {
        ...
        {
            _nodes = Enumerable.Range(0, conjMatrix.GetLength(0)).Select(i => i == 0 ? new Node { Label = "Коэффициенты эластичности" } : new Node { Label = coefficients[i - 1].Item1 + "\n\n" + Math.Abs(Math.Round(coefficients[i - 1].Item2, 2)).ToString() }).ToList();
