...
class QueenPath {
    ...
    public static void setBase(long[][] dp, int[][] parentX, int[][] parentY, long[][] cost, Point[] minElementInRow, Point[] minElementInColumn, int n, int m, Point[][] minElementInDiagonal) {
        dp[1][1] = cost[1][1];
        minElementInRow[1] = new Point(dp[1][1], 1, 1);
        minElementInColumn[1] = new Point(dp[1][1], 1, 1);
        minElementInDiagonal[1][1] = new Point(dp[1][1], 1, 1);
        ...
        for (int j = 2; j <= m; j++) {
            ...
            minElementInDiagonal[1][j] = new Point(dp[1][j], j, 1);
        }
        ...
        for (int i = 2; i <= n; i++) {
            ...
            minElementInDiagonal[i][1] = new Point(dp[i][1], 1, i);
        }
    }

    public static long calcMinCost(long[][] dp, int[][] parentX, int[][] parentY, long[][] cost, Point[] minElementInRow, Point[] minElementInColumn, int n, int m, Point[][] minElementInDiagonal) {
        for (int i = 2; i <= n; i++) {
            for (int j = 2; j <= m; j++) {
                long min = Math.min(Math.min(minElementInRow[i].val, minElementInColumn[j].val), minElementInDiagonal[i - 1][j - 1].val);
                dp[i][j] = min + cost[i][j];

                if (minElementInRow[i].val + cost[i][j] == dp[i][j]) {
                    parentX[i][j] = minElementInRow[i].x;
                    parentY[i][j] = minElementInRow[i].y;
                } else if (minElementInColumn[j].val + cost[i][j] == dp[i][j]) {
                    parentX[i][j] = minElementInColumn[j].x;
                    parentY[i][j] = minElementInColumn[j].y;
                } else {
                    parentX[i][j] = minElementInDiagonal[i - 1][j - 1].x;
                    parentY[i][j] = minElementInDiagonal[i - 1][j - 1].y;
                }
                ...
                if (dp[i][j] < minElementInDiagonal[i - 1][j - 1].val) {
                    minElementInDiagonal[i][j] = new Point(dp[i][j], j, i);
                } else {
                    minElementInDiagonal[i][j] = minElementInDiagonal[i - 1][j - 1];
                }
            }
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        Point[][] minElementInDiagonal = new Point[n + 1][m + 1];
        int[][] parentX = new int[n + 1][m + 1];
        int[][] parentY = new int[n + 1][m + 1];

        fillCostArray(cost);
        setBase(dp, parentX, parentY, cost, minElementInRow, minElementInColumn, n, m, minElementInDiagonal);
        out.print(calcMinCost(dp, parentX, parentY, cost, minElementInRow, minElementInColumn, n, m, minElementInDiagonal));
