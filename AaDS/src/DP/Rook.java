...
public class Rook {
    ...
    public static long getWaysCount(long[][] dp, long[][] partSumRows, long[][] partSumColumns, int n, int m) {
        for (int i = 2; i <= n; i++) {
            for (int j = 2; j <= m; j++) {
                ...
                dp[i][j] = partSumRows[i][j] + partSumColumns[i][j];
                if(dp[i][j] >= div){
    ...
    public static void main(String[] args) {
        ...
        System.out.println(getWaysCount(dp, partSumRows, partSumColumns, n, m));
    }
}
